...
import { getReportDetails, getSalaryPayments } from "../../redux/slices/ERPDashboardSlice";
...
export function SalaryPaymentTab() {
  ...
  const { salaryPayments, loading, errors } = useSelector(
...
const fetchData = async () => {
      try {
        await dispatch(getSalaryPayments()).unwrap();
    ...
    useEffect(() => {
     ...
     useEffect(() => {
    console.log("Report companyName Updated:", salaryPayments);
    }, [salaryPayments]);
 const DummyData = [
  {
  ...
  const filteredData = useMemo(() => {
        if (!salaryPayments || !Array.isArray(salaryPayments)) return [];
        if (!search.trim()) return salaryPayments;
