...
import { getOtherPayments, getReportDetails } from "../../redux/slices/ERPDashboardSlice";
...
export function OtherPaymentTab() {
  ...
  const { otherPayments, loading, errors } = useSelector(
...
const fetchData = async () => {
      try {
        await dispatch(getOtherPayments()).unwrap();
    ...
    useEffect(() => {
     ...
     useEffect(() => {
    console.log("Report companyName Updated:", otherPayments);
    }, [otherPayments]);
 const DummyData = [
  {
  ...
  // Filtered data
   const filteredData = useMemo(() => {
         if (!otherPayments || !Array.isArray(otherPayments)) return [];
         if (!search.trim()) return otherPayments;
