...
import { getLoanPayments, getReportDetails } from "../../redux/slices/ERPDashboardSlice";
...
export function LoanPaymentTab() {
  ...
  const { loanPayments, loading, errors } = useSelector(
...
const fetchData = async () => {
      try {
        await dispatch(getLoanPayments()).unwrap();
    ...
    useEffect(() => {
     ...
     useEffect(() => {
    console.log("Report companyName Updated:", loanPayments);
    }, [loanPayments]);
 const DummyData = [
  {
  ...
  const filteredData = useMemo(() => {
        if (!loanPayments || !Array.isArray(loanPayments)) return [];
        if (!search.trim()) return loanPayments;
